...
class Staging_Parts_Window:
    ...
    def __init__(self, gui_applicaiton, name, connection, engine, root, frame2, token):
    ...
    def update_view(self):
        # Updates view to go back to Tote to scan remaining products
        update_list = requests.get(
            'https://warehouse-api-wnnm.onrender.com/updated-stage-view',
            params={'tote_zone': self.tote_zone, 'product_id': self.product_id},
            headers={"Auth": self.token}
        ).json()

        if(len(update_list) == 0):
            # If there are no more products to be scan from Tote, go back to the main view
            tote_result_dic = requests.get(
                'https://warehouse-api-wnnm.onrender.com/check-tote-empty',
                params={'tote_zone': self.tote_zone},
                headers={"Auth": self.token}
            ).json()
            tote_result = tote_result_dic[0][0]
            ...
            quantity = update_list[0][1]
            product_name = update_list[0][2]
    ...
    def stage_part(self, event):
        ...
        # Check if inputted Stage name is valid
        stage_result_dic = requests.get(
            'https://warehouse-api-wnnm.onrender.com/check-stage',
            params={'stage_scanned': stage_scanned},
            headers={"Auth": self.token}
        ).json()
        stage_result = stage_result_dic[0][0]

        if(stage_result != 0 and (int(quantity_scanned) <= int(self.quantity) and int(stage_result) == 1)):
            # Move products from Tote to the Stage
            requests.get(
                'https://warehouse-api-wnnm.onrender.com/move-product-to-stage',
                params={'product_id': str(self.product_id), 'quantity_scanned': str(quantity_scanned),'tote_zone': self.tote_zone, 'stage_scanned': stage_scanned},
                headers={"Auth": self.token}
            )
    ...
    def load_product_info(self):
        ...
        # Returns view that allows the user to see how many products need to be scan to Stage from Tote
        load_product_view_dic = requests.get(
            'https://warehouse-api-wnnm.onrender.com/load-product-view',
            params={'tote_zone': self.tote_zone, 'product_id': self.product_id},
            headers={"Auth": self.token}
        ).json()

        self.quantity = load_product_view_dic[0][1]
        product_name = load_product_view_dic[0][2]
    ...
    def check_product_scanned(self, event):
        ...
        # Check if inputted product id is correct
        valid_product_dic = requests.get(
            'https://warehouse-api-wnnm.onrender.com/check-valid-product',
            params={'product_id': str(self.product_id)},
            headers={"Auth": self.token}
        ).json()
        product_check = valid_product_dic[0][0]
        
        if(product_check == 1):
            # Check if inputted product quantity is not more than the number required to be scan
            product_count_dic = requests.get(
                'https://warehouse-api-wnnm.onrender.com/check-product-count',
                params={'tote_zone': self.tote_zone, 'product_id': self.product_id},
                headers={"Auth": self.token}
            ).json()

            product_count = product_count_dic[0][0]
    ...
    def check_tote_item_list(self, event):
        ...
        # Check inputted Tote name is valid
        tote_result_dic = requests.get(
        ...
        if(tote_result == 1):
            # Check if Tote has no products scan to its location
            empty_tote_dic = requests.get(
                'https://warehouse-api-wnnm.onrender.com/check-empty-tote',
                params={'tote_zone': tote_zone},
                headers={"Auth": self.token}
            ).json()
            tote_empty_result = empty_tote_dic[0][0]
